#![allow(unused)]
// I did not write all of this code, I have just modified it to my purposes.
// I plan on keeping the unit tests because they help verify that the
// modifications I make are not breaking.
...
impl ThreadPool {
    ...
    {
        let f = Box::new(f);
        let f = unsafe { std::mem::transmute::<Box<dyn FnBox + 'a>, Thunk<'static>>(f) };
...
mod test {
    ...
    fn test_no_fun_or_joy() {
        ...
        let p_t = pool.clone();
        thread::spawn(move || {
            (0..23).for_each(|_| p_t.execute(sleepy_function));
        });
